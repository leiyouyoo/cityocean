  ...
  GetAll(json: { searchText?: string, IsDelivered?: boolean, TransportationMode?: TransportationMode, Sorting?: string, MaxResultCount?: number, SkipCount?: number }) {
    return this.http.postJson('/CSP/Shipment/GetAllList', json)
      .pipe(map((data: any) => {
        ...
        temp.forEach(s => {
          try {
            s.shiperShow = s.routeDetails.shipperInfos.length && s.routeDetails.shipperInfos[0].shipperNetWorkInfo.streetAddress || '';
            s.consigneeShow = s.routeDetails.consigneeInfos.length && s.routeDetails.consigneeInfos[0].consigneeNetWorkInfo.streetAddress || '';
          } catch (e) {
            console.log(e);
          }
          s.state = s.status;

...
export class MyShipmentService {
    ...
    this._shipmentsList = [
      {
        ...
        shipperList: ['shipper', 'shipper', 'shipper'],
        consigneeList: ['consignee', 'consignee', 'consignee'],
        originPortId: 0,
        originPort: 'Ningbo Port',
        destinationPortId: 0,
        destinationPort: 'New York port ,NY,United',
        cargoReadyDates: ['2019-09-20T03:53:30.024Z'],
  ...
  GetAll(json: {
    ...
    return this.http.postJson('/CSP/Shipment/GetAllList', json).pipe(
      map((data: any) => {
        const temp = data.items;
        temp.forEach((s) => {
          try {
            s.shiperShow =
              (s.routeDetails.shipperInfos.length && s.routeDetails.shipperInfos[0].shipperNetWorkInfo.streetAddress) ||
              '';
            s.consigneeShow =
              (s.routeDetails.consigneeInfos.length &&
                s.routeDetails.consigneeInfos[0].consigneeNetWorkInfo.streetAddress) ||
              '';
          } catch (e) {
            console.log(e);
          }
  ...
  getShipmentLinkDetail(Id: string) {
    let params = { Id };
